...
async def handle_query(call: types.CallbackQuery):
    ...
    if data == 'menu':
        ...
        daily_videos = {
            0.5: "/root/bots/stars_magnat/0.5_1767819250926.mp4",
            1.0: "/root/bots/stars_magnat/1_1767819250926.mp4",
            1.5: "/root/bots/stars_magnat/1.5_1767819250926.mp4",
            2.0: "/root/bots/stars_magnat/2_1767819250926.mp4",
            2.5: "/root/bots/stars_magnat/2.5_1767819250926.mp4",
            3.0: "/root/bots/stars_magnat/3_1767819250926.mp4",
            5.0: "/root/bots/stars_magnat/5_1767819250926.mp4",
            10.0: "/root/bots/stars_magnat/10_1767819250926.mp4",
            20.0: "/root/bots/stars_magnat/20_1767819250926.mp4",
            "JACKPOT": "/root/bots/stars_magnat/Джекпот__1767819250926.mp4"
        }

        # Хардкод file_id для анимаций/видео (если ранее уже отправлялось)
        hardcoded_file_ids = {
            "0.5": "CgACAgIAAxkBAAJZQmlrZtugVHIu-gI9SR_lhHEWs1PrAAIvmQAChQ9hS-HzTcTNqayyOAQ",
        ...
        cached_id = cache.get(str(video_key)) or hardcoded_file_ids.get(str(video_key))

        # Отправка анимации/видео
        if cached_id:
            try:
                sent = await bot.send_animation(
                    chat_id=call.from_user.id,
                    animation=cached_id,
                    caption="🎡 Открываем кейс..."
                )
            except Exception:
                cached_id = None
        ...
        if not cached_id:
            ...
            if local_path and os.path.exists(local_path):
                sent = await bot.send_animation(
                    chat_id=call.from_user.id,
                    animation=types.FSInputFile(local_path),
                    caption="🎡 Открываем кейс..."
                )
                if sent.animation:
                    cache[str(video_key)] = sent.animation.file_id
                    await set_user_state(777, json.dumps(cache))
            else:
                await bot.send_message(call.from_user.id, "🎡 Открываем кейс...")

        # Задержка 7 секунд перед выводом результата
