    ...
    if data == 'menu':
        ...
        async with db_pool.acquire() as conn:
            if user_choice == bot_choice:
                ...
                # Учитываем победу
                await conn.execute('UPDATE users SET wins = wins + 1 WHERE user_id = $1', user_id_int)
                await log_action(user_id_int, 'knb_win', delta, {'choice': user_choice, 'bot': bot_choice, 'bet': bet})
            else:
                delta = -bet
                result_text = f"💥 <b>Ты проиграла...</b>\nПроиграно <b>{bet} ⭐️</b>"
                # Учитываем поражение
                await conn.execute('UPDATE users SET losses = losses + 1 WHERE user_id = $1', user_id_int)
                await log_action(user_id_int, 'knb_loss', bet, {'choice': user_choice, 'bot': bot_choice})
