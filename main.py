...
async def handle_query(call: types.CallbackQuery):
    ...
    if data == 'menu':
        ...
        # Кэшируем file_id в state (как раньше у вас использовался id=777)
        file_id_raw = await get_user_state(777)
        import json
        try:
            cache = json.loads(file_id_raw) if file_id_raw else {}
        except:
            cache = {}

        cached_id = cache.get(str(video_key))
        ...
        try:
            if cached_id:
                print(f"[OPEN_CASE] Using cached file_id for {video_key}: {cached_id}")
                # Пытаемся отправить как animation (Telegram принимает file_id для нужного типа)
                try:
                    sent = await bot.send_animation(chat_id=target_chat, animation=cached_id, caption="🎡 Открываем кейс...")
                except Exception:
                    # если отправка как animation не проходит, пробуем как video
                    sent = await bot.send_video(chat_id=target_chat, video=cached_id, caption="🎡 Открываем кейс...")
            else:
                local_path = daily_videos.get(video_key)
                if local_path:
                    full_path = local_path if os.path.isabs(local_path) else os.path.join(base_dir, local_path)
                    exists = os.path.exists(full_path)
                    print(f"[OPEN_CASE] video_key={video_key} full_path={full_path} exists={exists}")
                    if exists:
                        ext = os.path.splitext(full_path)[1].lower()
                        if ext in ('.mp4', '.mov', '.mkv'):
                            sent = await bot.send_video(chat_id=target_chat, video=types.InputFile(full_path), caption="🎡 Открываем кейс...")
                            file_obj = getattr(sent, 'video', None)
                        else:
                            # gif or other animation
                            sent = await bot.send_animation(chat_id=target_chat, animation=types.InputFile(full_path), caption="🎡 Открываем кейс...")
                            file_obj = getattr(sent, 'animation', None)

                        # Кэшируем file_id для повторной отправки
                        if file_obj:
                            fid = file_obj.file_id
                            cache[str(video_key)] = fid
                            try:
                                await set_user_state(777, json.dumps(cache))
                                print(f"[OPEN_CASE] Cached file_id for {video_key}: {fid}")
                            except Exception as e:
                                print(f"[OPEN_CASE] Failed to cache file_id: {e}")
                    else:
                        print(f"[OPEN_CASE] Local file not found: {full_path}. Sending text fallback.")
