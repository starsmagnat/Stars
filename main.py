...
async def handle_query(call: types.CallbackQuery):
    ...
    if data == 'menu':
        ...
        # Пути к видео (файлы находятся в корне или в attached_assets)
        # Судя по ls, файлы в корне, но в коде был префикс attached_assets/
        daily_videos = {
        ...
        video_key = "JACKPOT" if is_jackpot else f"{amount:.1f}"
        
        # Хардкод file_id (чтобы не зависеть от файлов на диске)
        hardcoded_file_ids = {
        ...
        cache_key = f"file_id_{video_key}"
        ...
        if cached_id:
            try:
                sent_video = await call.message.answer_animation(
                    cached_id,
                    caption="🎡 Открываем кейс..."
                )
            except:
                cached_id = None # Если file_id невалиден (например, сменили бота)
        ...
        if not cached_id:
            if video_path and os.path.exists(video_path):
                ...
                if sent_video.animation:
                    cache[str(video_key)] = sent_video.animation.file_id
                    await set_user_state(777, json.dumps(cache))
            else:
                await call.message.answer("🎡 Открываем кейс...")
