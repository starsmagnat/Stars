...
async def handle_query(call: types.CallbackQuery):
    ...
    if data == 'menu':
        ...
        # 1. Пытаемся отправить по file_id
        if cached_id:
            try:
        ...
        # 2. Если не получилось — отправляем файл
        if not sent_msg and video_path and os.path.exists(video_path):
            sent_msg = await call.message.answer_animation(
        ...
        if not sent_msg:
            ...
            # 4. Универсально сохраняем file_id
            file_id = None
