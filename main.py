...
async def admin_jackpot_cmd(message: types.Message):
    ...
    await message.answer(f"💰 Текущая сумма джекпоталох: {amount:.2f} ⭐️")
...
async def handle_query(call: types.CallbackQuery):
    ...
    if data == 'menu':
        ...
        print(f"[DEBUG] Кнопка открытия кейса нажата! user_id={user_id_int}")
        # Получаем данные пользователя и проверяем КД
        user_data = await get_user(user_id_int)
        now = time.time()
        if now - user_data['last_bonus'] < 86400:
            print(f"[DEBUG] КД не прошло для пользователя {user_id_int}")
        ...
        hardcoded_file_ids = {
            ...
            "3.0": "ВАШ_FILE_ID_3.0",
            "5.0": "ВАШ_FILE_ID_5.0",
            "10.0": "ВАШ_FILE_ID_10.0",
            "20.0": "ВАШ_FILE_ID_20.0",
            "JACKPOT": "ВАШ_FILE_ID_JACKPOT"
        }

        # Приводим к строке с одним знаком после запятой для совпадения с ключами
        if is_jackpot:
            video_key = "JACKPOT"
        else:
            video_key = f"{float(amount):.1f}"
        ...
        try:
            if cached_id and not cached_id.startswith("ВАШ_FILE_ID"):
                print(f"[OPEN_CASE] Attempting to send media using hardcoded file_id: {cached_id}")
                try:
                    await bot.send_animation(chat_id=target_chat, animation=cached_id, caption="🎡 Открываем кейс...")
                    print(f"[OPEN_CASE] Successfully sent as animation (file_id: {cached_id})")
                except Exception as e_anim:
                    print(f"[OPEN_CASE] Failed to send as animation, trying as video. Error: {e_anim}")
                    try:
                        await bot.send_video(chat_id=target_chat, video=cached_id, caption="🎡 Открываем кейс...")
                        print(f"[OPEN_CASE] Successfully sent as video (file_id: {cached_id})")
                    except Exception as e_vid:
                        print(f"[OPEN_CASE] CRITICAL: Failed to send media by file_id. Error: {e_vid}")
                ...
                if local_path and os.path.exists(local_path):
                    ...
                    print(f"[OPEN_CASE] Sending local file: {local_path} (ext: {ext})")
                    ...
                    print(f"[OPEN_CASE] No file_id and no local file for {video_key}")
                    await bot.send_message(target_chat, "🎡 Открываем кейс...")
        except Exception as e:
            print(f"[OPEN_CASE] Global error in media sending block: {e}")
            await bot.send_message(target_chat, "🎡 Открываем кейс...")
        ...
        markup = types.InlineKeyboardMarkup(inline_keyboard=[
            [types.InlineKeyboardButton(text="🔁 Ещё раз", callback_data='knb_repeat_bet')],
            [types.InlineKeyboardButton(text="✏️ Изменить ставку", callback_data='change_bet_input')],
            [types.InlineKeyboardButton(text="🎯 К мини-играм", callback_data='games')],
            [types.InlineKeyboardButton(text="🏠 В меню", callback_data='menu')]
        ])
...
async def main():
    ...
    try:
        # Сброс вебхука и удаление накопившихся обновлений
        await bot.delete_webhook(drop_pending_updates=True)
        print("[BOT] Webhook deleted, pending updates dropped")
...
if __name__ == "__main__":
    ...
    if len(sys.argv) > 1 and sys.argv[1] == "webhook":
        ...
        async def main_webhook():
            ...
            print(f"хееееемBot started on port {port} with webhook")
