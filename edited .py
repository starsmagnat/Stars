        ...
        async with db_pool.acquire() as conn:
            ...
            stats = await conn.fetchrow(f"""
                SELECT 
                    ...
                    (SELECT SUM(cases_opened) FROM users) as total_cases_all,
                    (SELECT SUM(wins) FROM users) as total_wins_all,
                    (SELECT SUM(losses) FROM users) as total_losses_all
                FROM action_logs 
...
async def info_command_handler(message: types.Message):
    ...
    async with db_pool.acquire() as conn:
        ...
        text = (
            ...
            f"🎁 Кейсов открыто: {user_row['cases_opened'] or 0}\n\n"
