...
# Временное хранилище для предотвращения дабл-кликов
last_clicks = {}
...
async def roll_fortune_wheel(user_id: int):
    # Призы и вероятности
    prizes = [
        (0.5, 1000), (1.0, 500), (1.5, 300), (2.0, 100), 
        (2.5, 60), (3.0, 20), (5.0, 14), (10.0, 4), 
        (20.0, 1), ("JACKPOT", 1)
    ]
    ...
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            # ИСПРАВЛЕНИЕ: Добавляем cases_opened в запрос
            row = await conn.fetchrow('SELECT last_bonus, username, name, cases_opened FROM users WHERE user_id = $1 FOR UPDATE', user_id)
            if not row: 
            ...
            # ИСПРАВЛЕНИЕ: Проверяем реферальную систему только если это ПЕРВЫЙ кейс пользователя
            if row['cases_opened'] == 0:  # <-- ВАЖНОЕ ИЗМЕНЕНИЕ!
                ref_id = await conn.fetchval('SELECT referrer_id FROM referral_connections WHERE user_id = $1', user_id)
                if ref_id:
                    print(f"[REFERRAL] First spin! Rewarding referrer {ref_id} for user {user_id}")
                    
                    # Начисляем награду рефереру
                    await conn.execute('UPDATE users SET balance = balance + 2, refs = refs + 1 WHERE user_id = $1', ref_id)
                    await log_action(ref_id, 'referral_reward', 2, {'referred_user': user_id})
                    
                    # Добавляем в турнирную статистику
                    ...
                    try:
                        await bot.send_message(
                            ref_id, 
                            f"🎁 <b>Ваш реферал открыл свой первый кейс!</b>\n\n"
                            f"👤 Реферал: {row['name']}\n"
                            f"💰 Вам начислено: <b>2 ⭐️</b>\n\n"
                            f"Поздравляем!", 
                            parse_mode='HTML'
                        )
                    except Exception as e:
                        print(f"[REFERRAL] Failed to send notification to referrer {ref_id}: {e}")
            ...
            await conn.execute(
                'UPDATE users SET balance = balance + $1, last_bonus = $2, cases_opened = cases_opened + 1 WHERE user_id = $3',
...
async def roll_fortune_wheel(user_id: int):
    ...
    # 0.5 звезд -- 50% -> 1000 (было 35%)
    # 1 звезда -- 25% -> 500
    # 1.5 звезды -- 15% -> 300
    # 2 звезды -- 5% -> 100 (было 10%)
    # 2.5 звезды -- 3% -> 60 (было 8%)
    # 3 звезды -- 1% -> 20 (было 4%)
    # 5 звезд -- 0.7% -> 14 (было 2%)
    # 10 звезд -- 0.2% -> 4 (было 0.6%)
    # 20 звезд -- 0.05% -> 1 (было 0.3%)
    # Джекпот -- 0.05% -> 1 (без изменений)
    prizes = [
        (0.5, 1000), (1.0, 500), (1.5, 300), (2.0, 100), 
        (2.5, 60), (3.0, 20), (5.0, 14), (10.0, 4), 
        (20.0, 1), ("JACKPOT", 1)
                    ...
                    if ref_id:
                        ...
                        try:
                            await bot.send_message(
                                ref_id, 
                                f"🎁 <b>Ваш реферал открыл свой первый кейс!</b>\n\n"
                                f"Поздравляем! Вам начислено <b>2 ⭐️</b>", 
                                parse_mode='HTML'
                            )
                        except: pass
...
async def get_case_subscriptions():
    async with db_pool.acquire() as conn:
        rows = await conn.fetch('SELECT url, channel_id, button_name FROM case_subscriptions WHERE is_active = TRUE')
        return [dict(row) for row in rows]

async def add_case_subscription(url, channel_id, button_name):
    async with db_pool.acquire() as conn:
        await conn.execute(
            'INSERT INTO case_subscriptions (url, channel_id, button_name) VALUES ($1, $2, $3)',
            url, channel_id, button_name
        )

@dp.message(Command("get_file_id"))
async def admin_get_file_id(message: types.Message):
    if not is_admin(message.from_user.id): return
    await message.reply("📤 Отправьте мне GIF или видео, чтобы я выдал вам его <code>file_id</code>", parse_mode='HTML')
    await set_user_state(message.from_user.id, 'awaiting_video_for_id')

@dp.message(Command("add_case_sub"))
async def admin_add_case_sub(message: types.Message):
    if not is_admin(message.from_user.id): return
    try:
        # /add_case_sub [url] [channel_id or 0] [button_name]
        parts = message.text.split(maxsplit=3)
        if len(parts) < 4:
            await message.reply("❌ Формат: /add_case_sub [URL] [ID канала или 0] [Название кнопки]")
            return
        ...
        await add_case_subscription(url, channel_id if channel_id != 0 else None, button_name)
        await message.reply(f"✅ Подписка на кейс добавлена: {button_name}")
    except Exception as e:
        await message.reply(f"❌ Ошибка: {e}")

@dp.message(Command("del_case_sub"))
async def admin_del_case_sub(message: types.Message):
    if not is_admin(message.from_user.id): return
    async with db_pool.acquire() as conn:
        await conn.execute('DELETE FROM case_subscriptions')
        await message.reply("✅ Все подписки для кейсов удалены")
    ...
    try:
        ...
        async with db_pool.acquire() as conn:
            async with conn.transaction():
                ...
                # ИСПРАВЛЕНИЕ: Не начисляем награду сразу, только сохраняем связь
                # Награда будет начислена когда реферал откроет первый кейс
                await conn.execute(
                    'INSERT INTO referral_connections (user_id, referrer_id) VALUES ($1, $2) ON CONFLICT (user_id) DO NOTHING',
                    user_id, ref_id
                )
                print(f"[REFERRAL] Connection saved for user {user_id} with referrer {ref_id}. Reward pending first case open.")

        try:
            await bot.send_message(
                ref_id,
                f"👥 <b>У вас новый реферал — {user_name or 'Новый пользователь'}!</b>\n\n"
                f"💰 Награда (2 ⭐️) будет начислена когда он откроет свой первый Ежедневный кейс.",
                parse_mode='HTML'
            )
...
async def use_promo(user_id: int, code: str):
    async with db_pool.acquire() as conn:
        async with conn.transaction():
            ...
            await conn.execute(
                'UPDATE promos SET uses = uses - 1 WHERE UPPER(code) = UPPER($1)',
                code
            )
...
async def stats_command_handler(message: types.Message):
    ...
    args = message.text.split()
    if len(args) < 3:
        await message.reply("❌ Формат: /stats [число] [hours/days]\nПример: /stats 24 hours")
        return
    ...
    try:
        ...
        if unit.startswith('hour'):
            ...
            await message.reply("❌ Используйте hours или days")
            return

        async with db_pool.acquire() as conn:
            # ИСПРАВЛЕНИЕ: Используем корректные имена полей
            stats = await conn.fetchrow(f"""
                SELECT 
                    COUNT(DISTINCT user_id) as active_users,
                    COUNT(*) FILTER (WHERE action_type IN ('casino_bet', 'knb_bet')) as total_games,
                    COALESCE(SUM(amount) FILTER (WHERE action_type IN ('casino_bet', 'knb_bet')), 0) as total_staked,
                    COALESCE(SUM(amount) FILTER (WHERE action_type IN ('casino_win', 'knb_win', 'casino_result')), 0) as total_won,
                    ...
                    (SELECT COALESCE(SUM(wins), 0) FROM users) as total_wins_all,
                    (SELECT COALESCE(SUM(losses), 0) FROM users) as total_losses_all,
                    (SELECT COALESCE(SUM(cases_opened), 0) FROM users) as total_cases_all
                FROM action_logs 
            ...
            game_stats = await conn.fetch(f"""
                SELECT 
                    COALESCE(details->>'game', 'unknown') as game, 
                    COUNT(*) as count,
                    COUNT(*) FILTER (WHERE action_type IN ('casino_win', 'knb_win', 'casino_result') AND (details->>'outcome' = 'win' OR details->>'outcome' IS NULL)) as wins,
                    COUNT(*) FILTER (WHERE action_type IN ('casino_loss', 'knb_loss') OR (action_type = 'casino_result' AND details->>'outcome' = 'loss')) as losses,
                    COALESCE(SUM(amount) FILTER (WHERE action_type IN ('casino_bet', 'knb_bet')), 0) as staked,
                    COALESCE(SUM(amount) FILTER (WHERE action_type IN ('casino_win', 'knb_win', 'casino_result') AND (details->>'outcome' = 'win' OR details->>'outcome' IS NULL)), 0) as won
                FROM action_logs 
                WHERE created_at > NOW() - INTERVAL {interval}
                AND action_type IN ('casino_bet', 'knb_bet', 'casino_win', 'knb_win', 'casino_result', 'casino_loss', 'knb_loss')
                GROUP BY COALESCE(details->>'game', 'unknown')
            """)
            ...
            text += f"✅ Побед: {stats['total_wins_all'] or 0} | ❌ Поражений: {stats['total_losses_all'] or 0}\n"
            text += f"🎁 Кейсов открыто: {stats['total_cases_all'] or 0}\n"
            ...
            for g in game_stats:
                ...
                game_name = g['game'].capitalize() if g['game'] != 'unknown' else 'Неизвестно'
                text += f"🔹 {game_name}: {g['count']} игр ({g['wins']}В/{g['losses']}П) | Доход: {g_profit:.1f}⭐\n"
    ...
    async with db_pool.acquire() as conn:
        ...
        stats = await conn.fetchrow("""
            SELECT 
                COUNT(*) FILTER (WHERE action_type IN ('casino_bet', 'knb_bet')) as total_games,
                COALESCE(SUM(amount) FILTER (WHERE action_type IN ('casino_bet', 'knb_bet')), 0) as total_staked,
                COALESCE(SUM(amount) FILTER (WHERE action_type IN ('casino_win', 'knb_win', 'casino_result') AND (details->>'outcome' = 'win' OR details->>'outcome' IS NULL)), 0) as total_won,
                COUNT(*) FILTER (WHERE action_type = 'promo') as promos_count,
                COALESCE(SUM(amount) FILTER (WHERE action_type = 'withdraw_request'), 0) as total_withdrawn,
                COUNT(*) FILTER (WHERE action_type = 'support_request') as support_count,
                COUNT(*) FILTER (WHERE action_type = 'case_open') as cases_opened_count
        ...
        user_game_stats = await conn.fetch("""
            SELECT 
                ...
                COALESCE(SUM(amount) FILTER (WHERE action_type IN ('casino_win', 'knb_win', 'casino_result') AND (details->>'outcome' = 'win' OR details->>'outcome' IS NULL)), 0) as won
            FROM action_logs 
            WHERE user_id = $1 AND action_type IN ('casino_bet', 'knb_bet', 'casino_win', 'knb_win', 'casino_result', 'casino_loss', 'knb_loss')
            GROUP BY COALESCE(details->>'game', 'unknown')
        """, uid)
        ...
        text = (
            ...
            f"🎁 Кейсов открыто: {stats['cases_opened_count'] or 0}\n\n"
            f"📊 <b>Игровая активность:</b>\n"
            f"🎮 Всего игр: {stats['total_games'] or 0}\n"
            f"✅ Побед: {user_row['wins'] or 0} | ❌ Поражений: {user_row['losses'] or 0}\n"
        ...
        for ugs in user_game_stats:
            ...
            game_name = ugs['game'].capitalize() if ugs['game'] != 'unknown' else 'Неизвестно'
            text += f"🔹 {game_name}: {ugs['count']} игр ({ugs['wins']}В/{ugs['losses']}П) | {u_profit:+.1f}⭐\n"
            ...
            try:
                ...
                if ref_user and ref_id_int != uid:
                    ...
                    print(f"[REFERRAL] Connection saved via start: {uid} invited by {ref_id_int}. Reward pending first case open.")
                        ...
                        # Уведомление пригласителю СРАЗУ при переходе
                    try:
                        await bot.send_message(
                            ref_id_int, 
                            f"👥 <b>У вас новый реферал — {message.from_user.first_name}!</b>\n\n"
                            f"Как только он откроет свой первый Ежедневный кейс, вам начислят <b>2 ⭐️</b>",
                            parse_mode='HTML'
                        )
                    except: pass
            except ValueError:
...
async def handle_query(call: types.CallbackQuery):
    # Защита от спама и конфликтов кнопок
    uid_num = call.from_user.id
    now = time.time()
    if uid_num in last_clicks:
        if now - last_clicks[uid_num] < 0.4:
            await call.answer()
            return
    last_clicks[uid_num] = now
    ...
    # Сброс состояния при переходе между основными разделами
    if data in ['menu', 'games', 'knb', 'casino', 'dice', 'basket', 'bowling', 'daily']:
        user_states[user_id] = None
        await set_user_state(user_id_int, None)
    if data == 'check_sub':
        if await check_subscription(call.from_user.id):
            try:
                await call.message.delete()
            ...
            if ref_id:
                ...
                if is_new_user:
                    ...
                    if ref_user and ref_id != user_id_int:
                        ...
                        # Уведомление пригласителю (если зашел через подписку)
                        try:
                            await bot.send_message(
                                ref_id, 
                                f"👥 <b>У вас новый реферал — {call.from_user.first_name}!</b>\n\n"
                                f"Как только он откроет свой первый Ежедневный кейс, вам начислят <b>2 ⭐️</b>",
                                parse_mode='HTML'
                            )
                        except: pass
    ...
    if data == 'menu':
        ...
        # Проверка обязательных подписок для кейса
        case_subs = await get_case_subscriptions()
        if case_subs:
            unsubscribed = []
            for sub in case_subs:
                if sub['channel_id']:
                    try:
                        member = await bot.get_chat_member(chat_id=sub['channel_id'], user_id=user_id_int)
                        if member.status in ['left', 'kicked']:
                            unsubscribed.append(sub)
                    except:
                        # Если не можем проверить (бот не админ), считаем что нужно подписаться
                        unsubscribed.append(sub)
                else:
                    # Это просто ссылка (например реферальная), её проверить нельзя, но нужно показать
                    # Однако по ТЗ: "пользователь просто перешел по ней без проверки на подписку"
                    # Мы её добавим в кнопки, но не будем блокировать прогресс если channel_id = 0
                    pass

            if unsubscribed:
                markup = types.InlineKeyboardMarkup(inline_keyboard=[])
                for sub in case_subs:
                    markup.inline_keyboard.append([types.InlineKeyboardButton(text=sub['button_name'], url=sub['url'])])
                markup.inline_keyboard.append([types.InlineKeyboardButton(text="✅ Проверить и открыть", callback_data='daily')])
                markup.inline_keyboard.append([types.InlineKeyboardButton(text="◀️ Назад", callback_data='menu')])

                await bot.send_photo(
                    chat_id, images['bonus'],
                    caption="❌ <b>Для открытия кейса необходимо выполнить условия:</b>",
                    reply_markup=markup,
                    parse_mode='HTML'
                )
                return
        ...
        # Получаем данные пользователя и проверяем КД
        user_data = await get_user(user_id_int)
        now = time.time()
        if now - user_data['last_bonus'] < 86400:
            await call.answer("⏱ Кейс уже открыт сегодня. Возвращайся завтра!", show_alert=True)
            return

        # Получаем результат прокрута
        roll_res, error = await roll_fortune_wheel(user_id_int)
        if error:
            if error == "cooldown":
                await call.answer("⏱ Кейс уже открыт сегодня. Возвращайся завтра!", show_alert=True)
            else:
                await call.answer(f"❌ Ошибка: {error}", show_alert=True)
            return

        amount = roll_res['amount']
        is_jackpot = roll_res['is_jackpot']

        # Пути к локальным видео (можно изменить на абсолютные пути)
        daily_videos = {
            0.5: "0.5_1767819250926.mp4",
            1.0: "1_1767819250926.mp4",
            1.5: "1.5_1767819250926.mp4",
            2.0: "2_1767819250926.mp4",
            2.5: "2.5_1767819250926.mp4",
            3.0: "3_1767819250926.mp4",
            5.0: "5_1767819250926.mp4",
            10.0: "10_1767819250926.mp4",
            20.0: "20_1767819250926.mp4",
            "JACKPOT": "Джекпот__1767819250926.mp4"
        }

        # file_id хардкод (вставьте сюда ваши ID)
        hardcoded_file_ids = {
            "0.5": "BAACAgIAAxkBAAJY4WlqlFPH0xB8Yedwtonx75YO6IpRAAIPngAChQ9ZS9Ww2AiLyvS7OAQ",
            "1.0": "CgACAgIAAxkBAAJY5WlqlJkmtwbhsMxJ7NQlk6dNfz6LAAIWngAChQ9ZS1p5BhGoDGLgOAQ",
            "1.5": "CgACAgIAAxkBAAJY6WlqlOqdHWGmXGiYQNpKC9ExfN3VAAIYngAChQ9ZSybbkRfWNQABwzgE",
            "2.0": "CgACAgIAAxkBAAJY7WlqlWDKNWVAZ86vDDF1M5NKjiufAAIengAChQ9ZSydZRVSDrVFqOAQ",
            "2.5": "CgACAgIAAxkBAAJY8Wlqld3VFmazRZ_nbS0sm9MWas1DAAIhngAChQ9ZS7q02uOyWXdLOAQ",
            "3.0": "CgACAgIAAxkBAAJY9WlqlmUdm8Zrp5NLGpBcdoi7vEv0AAIlngAChQ9ZS9roKNvRB41ROAQ",
            "5.0": "CgACAgIAAxkBAAJY-Wlqlrwa8YlevxVPYMebDrRrZCe1AAIpngAChQ9ZSy0uQbMTnWclOAQ",
            "10.0": "CgACAgIAAxkBAAJZAWlql49Jtp54kRyojdCadpEKcEGAAAIvngAChQ9ZS7qgqm0NB92QOAQ",
            "20.0": "CgACAgIAAxkBAAJZBWlql86Mw64Pwq-0Cdr2D9_E5fmwAAIxngAChQ9ZS2GaqRE8vCj1OAQ",
            "JACKPOT": "CgACAgIAAxkBAAJZCWlqmAqkLM3nI_tRyxr56rFld4kxAAI2ngAChQ9ZSzeWLETCVJrkOAQ"
        }

        video_key = "JACKPOT" if is_jackpot else f"{float(amount):.1f}"
        cached_id = hardcoded_file_ids.get(video_key)

        # Отправляем анимацию / видео — используем chat_id (тут call.message.chat.id)
        target_chat = call.message.chat.id

        try:
            if cached_id and not cached_id.startswith("ВАШ_FILE_ID"):
                print(f"[OPEN_CASE] Using hardcoded file_id for {video_key}: {cached_id}")
                try:
                    await bot.send_animation(chat_id=target_chat, animation=cached_id, caption="🎡 Открываем кейс...")
                except Exception:
                    await bot.send_video(chat_id=target_chat, video=cached_id, caption="🎡 Открываем кейс...")
            else:
                # Если хардкод не заполнен, пытаемся найти локально (для Replit)
                local_path = daily_videos.get(video_key)
                if local_path and os.path.exists(local_path):
                    ext = os.path.splitext(local_path)[1].lower()
                    if ext in ('.mp4', '.mov', '.mkv'):
                        await bot.send_video(chat_id=target_chat, video=types.FSInputFile(local_path), caption="🎡 Открываем кейс...")
                    else:
                        await bot.send_animation(chat_id=target_chat, animation=types.FSInputFile(local_path), caption="🎡 Открываем кейс...")
                else:
                    await bot.send_message(target_chat, "🎡 Открываем кейс...")
        except Exception as e:
            print(f"[OPEN_CASE] Error sending animation/video: {e}")
            await bot.send_message(target_chat, "🎡 Открываем кейс...")

        # Небольшая задержка перед показом результата (чтобы анимация успела проиграться)
        await asyncio.sleep(7)

        # Итоговое сообщение
        await log_action(user_id_int, 'case_open', amount, {'is_jackpot': is_jackpot})
        if is_jackpot:
            msg = (
                f"🎉🎉🎉 <b>ДЖЕКПОТ!!!</b> 🎉🎉🎉\n\n"
                f"💰 Вы выиграли весь джекпот: <b>{amount:.2f} ⭐️</b>\n\n"
                f"Легенда!"
            )
        else:
            msg = (
                f"🎁 <b>Ежедневный кейс открыт!</b>\n\n"
                f"Ваш выигрыш: <b>{amount} ⭐️</b>\n"
                f"Удача сегодня на вашей стороне! ✨\n\n"
                f"Возвращайтесь завтра за новой порцией удачи!"
            )

        # Отправка итогового фото с балансом/инфо
        ...
        markup = types.InlineKeyboardMarkup(inline_keyboard=[
            ...
            [types.InlineKeyboardButton(text="🎰 Слоты", callback_data='game_casino')],
        ...
        print(f"[DEBUG] Повторная ставка (knb) нажата пользователем {user_id_int}")
        chat_id = call.message.chat.id
        uid = str(user_id_int)

        # Анти-спам
        if await get_user_state(user_id_int) == 'playing_game':
            print(f"[DEBUG] Анти-спам (knb): игрок {user_id_int} уже в игре")
            await call.answer("⏳ Дождись окончания предыдущей игры!", show_alert=True)
            return
        
        await set_user_state(user_id_int, 'playing_game')
        ...
        await bot.send_photo(
            ...
            caption="🎰 <b>Добро пожаловать в слоты!</b>\n\n"
                    "💵 <b>Укажите вашу ставку (от 1 до 50 ⭐️), чтобы начать игру.</b>\n\n"
        ...
        print(f"[DEBUG] Повторная ставка (casino) нажата пользователем {user_id_int}")
        chat_id = call.message.chat.id
        uid = str(user_id_int)

        # Анти-спам: проверяем, нет ли уже запущенной игры
        if await get_user_state(user_id_int) == 'playing_game':
            print(f"[DEBUG] Анти-спам: игрок {user_id_int} уже в игре")
            await call.answer("⏳ Дождись окончания предыдущей игры!", show_alert=True)
            return
        ...
        async with db_pool.acquire() as conn:
            ...
            if outcome == 'win':
                ...
                await conn.execute('UPDATE users SET wins = wins + 1 WHERE user_id = $1', user_id_int)
                await log_action(user_id_int, 'casino_win', win, {'game': curr_game, 'bet': bet})
            else:
                await conn.execute('UPDATE users SET losses = losses + 1 WHERE user_id = $1', user_id_int)
                await log_action(user_id_int, 'casino_loss', bet, {'game': curr_game})
        ...
        if not isinstance(user_state, dict) or 'bet' not in user_state:
            user_state = await get_user_state(uid_int)
        ...
        await log_action(user_id_int, 'casino_bet', float(bet), {'game': 'knb'})
        ...
        if user_choice == bot_choice:
            ...
            await log_action(user_id_int, 'knb_draw', bet, {'choice': user_choice, 'bot': bot_choice})
            # Обновляем статистику
            async with db_pool.acquire() as conn:
                pass  # Ничья не считается ни победой, ни поражением
        elif win_map[user_choice] == bot_choice:
            # Победа
            win_amount = round(bet * 1.9, 2)  # ×1.9 от ставки
            await update_user_balance(uid_int, win_amount)
            result_text = f"🎉 <b>Ты победил!</b>\nТы заработал <b>+{win_amount} ⭐️</b>!"
            # Обновляем статистику побед
            async with db_pool.acquire() as conn:
                await conn.execute('UPDATE users SET wins = wins + 1 WHERE user_id = $1', user_id_int)
                await log_action(user_id_int, 'casino_result', win_amount, 
                               {'game': 'knb', 'bet': bet, 'outcome': 'win', 'choice': user_choice, 'bot': bot_choice})
        else:
            # Поражение
            ...
            async with db_pool.acquire() as conn:
                await conn.execute('UPDATE users SET losses = losses + 1 WHERE user_id = $1', user_id_int)
                await log_action(user_id_int, 'casino_result', 0, 
                               {'game': 'knb', 'bet': bet, 'outcome': 'loss', 'choice': user_choice, 'bot': bot_choice})
        ...
        print(f"[DEBUG] Повторная ставка (dice) нажата пользователем {user_id_int}")
        chat_id = call.message.chat.id
        uid = str(user_id_int)
        ...
        async with db_pool.acquire() as conn:
            if user_value > bot_value:
                ...
                await conn.execute('UPDATE users SET balance = balance + $1, wins = wins + 1 WHERE user_id = $2', Decimal(str(delta)), user_id_int)
            elif user_value == bot_value:
                result_text = "🤝 <b>Ничья!</b> Ставка возвращена."
                await update_user_balance(user_id_int, bet)
            else:
        ...
        # Сбрасываем состояние перед отправкой результата
        await set_user_state(user_id_int, None)
        ...
        print(f"[DEBUG] Повторная ставка (basket) нажата пользователем {user_id_int}")
        chat_id = call.message.chat.id
        uid = str(user_id_int)
        ...
        async with db_pool.acquire() as conn:
            if value in (4, 5):
                ...
                await conn.execute('UPDATE users SET balance = balance + $1, wins = wins + 1 WHERE user_id = $2', Decimal(str(win)), user_id_int)
            else:
                win = 0
        ...
        print(f"[DEBUG] Повторная ставка (bowling) нажата пользователем {user_id_int}")
        chat_id = call.message.chat.id
        uid = str(user_id_int)
...
async def handle_user_input(message: types.Message):
    ...
    if state == 'awaiting_promo':
        ...
        await log_action(uid_int, 'promo_use', 0, {'code': code, 'success': result['success']})
        ...
        # Логируем запрос в поддержку
        await log_action(uid_int, 'support_request', 0, {'text': (message.text or message.caption or "")[:100]})
        ...
        try:
            ...
            if not val_str:
                await message.reply("❌ <b>Введите корректное число!</b>", parse_mode='HTML')
                return
            amount = float(val_str)

            if amount < 50:
                await message.reply("❌ <b>Вывод доступен от 50 ⭐️</b>\nВведите другую сумму:", parse_mode='HTML')
                return

            balance = await get_user_balance(uid_int)
            if amount > balance:
                await message.reply(f"❌ <b>Недостаточно средств.</b>\nВаш баланс: <b>{balance} ⭐️</b>\nВведите доступную сумму:", parse_mode='HTML')
                return

            # Логируем попытку вывода
            await log_action(uid_int, 'withdraw_attempt', amount)
    ...
    # Обработка входящих сообщений в ожидании file_id (для админа)
    elif state == 'awaiting_video_for_id':
        if not is_admin(uid_int): return
        fid = None
        if message.animation: fid = message.animation.file_id
        elif message.video: fid = message.video.file_id

        if fid:
            await message.reply(f"✅ <b>File ID получен:</b>\n\n<code>{fid}</code>\n\nТеперь вы можете вставить его в код main.py.", parse_mode='HTML')
        else:
            await message.reply("❌ Это не видео и не анимация. Отправьте GIF или MP4.")
        ...
        try:
            ...
            # Анти-спам
            if await get_user_state(uid_int) == 'playing_game':
                await message.reply("⏳ Дождись окончания предыдущей игры!")
                return

            await set_user_state(uid_int, 'playing_game')
    ...
    # Повтор ставки
        ...
        try:
            ...
            if bet < 1 or bet > 50:
                await message.reply("❌ <b>Ставка должна быть от 1 до 50 ⭐️.</b>\nПопробуйте еще раз:", parse_mode='HTML')
                return

            balance = await get_user_balance(uid_int)
            if bet > balance:
                await message.reply(f"❌ <b>Недостаточно ⭐️ для ставки.</b>\nВаш баланс: <b>{balance} ⭐️</b>\nПопробуйте еще раз:", parse_mode='HTML')
                return
            ...
            await bot.send_message(message.chat.id, "🎲 <b>Бросаем два кубика: сначала твой кубик, потом кубик бота</b>", parse_mode="HTML")
            ...
            await asyncio.sleep(3)
            await bot.send_message(message.chat.id, "🤖 <b>Бросок бота:</b>", parse_mode="HTML")
            ...
            if user_dice > bot_dice:
                ...
                await update_user_balance(uid_int, bet)
                await log_action(uid_int, 'casino_result', float(bet), {'game': 'dice', 'bet': bet, 'outcome': 'draw'})
        ...
        try:
            ...
            if value in (4, 5):
                win = round(bet * 2)
                await update_user_balance(uid_int, win)
                await log_action(uid_int, 'casino_result', float(win), {'game': 'basket', 'bet': bet, 'outcome': 'win'})
                result_text = f"🎉 <b>Попадание!</b>\n\n Ты выигрываешь <b>{win}</b> ⭐️"
            else:
